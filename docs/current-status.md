...
- 상단 허브의 첫 줄은 이제 `즉시 출고 / 송장 입력 / 재확인 / 보류 / 차단` 행동 큐 카드로 보이고, 각 카드는 전체 건수·대표 사유·대표 주문 3건 미리보기를 함께 보여줍니다.
- 행동 큐 카드를 누르면 하단 원본 테이블은 같은 `decisionStatus` 기준으로 바로 좁혀지고, 배송 단계/이슈 필터는 그 아래의 탐색 레이어로 유지됩니다.
