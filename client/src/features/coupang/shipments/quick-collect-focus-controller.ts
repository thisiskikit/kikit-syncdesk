...
function resolveFallbackActiveSheet(input: {
  ...
  return {
    ...
    mirrorTotalRowCount: input.result.focusedRows.length,
    mirrorFilteredRowCount: input.result.focusedRows.length,
    activeTotalRowCount: input.result.focusedRows.length,
    activeFilteredRowCount: input.result.focusedRows.length,
    activeExclusionCounts: {
      retention_post_dispatch: 0,
      cancel_completed: 0,
      return_completed: 0,
      not_found_in_coupang: 0,
    },
