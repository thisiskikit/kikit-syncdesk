...
async function createStore() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "kikit-naver-bulk-price-"));
  const filePath = path.join(tempDir, "naver-bulk-price.json");
  tempDirs.push(tempDir);

  return {
    filePath,
    store: new NaverBulkPriceStore(filePath),
  };
}
...
describe("NaverBulkPriceStore legacy persistence", () => {
  it("persists saved source and rule presets to disk", async () => {
    ...
    const reloadedStore = new NaverBulkPriceStore(filePath);
