...
describe("coupang order service", () => {
  ...
  it("keeps the single-status lookup path when a status filter is provided", async () => {
    ...
    expect(getOrderSheetCalls()[0]?.[0].query.get("createdAtFrom")).toBe("2026-03-20+09:00");
    expect(getOrderSheetCalls()[0]?.[0].query.get("createdAtTo")).toBe("2026-03-25+09:00");
  ...
  it("merges per-status live order results when no status filter is provided", async () => {
    ...
    for (const [input] of getOrderSheetCalls()) {
      expect(input.query.get("createdAtFrom")).toBe("2026-03-20+09:00");
      expect(input.query.get("createdAtTo")).toBe("2026-03-25+09:00");
    }
    expect(result.source).toBe("live");
    expect(result.servedFromFallback).toBe(false);
    expect(result.nextToken).toBeNull();
